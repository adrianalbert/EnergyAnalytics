...
#   data_15 = read.csv(files_15[i]); data_15$date = as.POSIXct(data_15$date); 
  ...
  df$Season = 'Winter'
  df$Season[which(df$Month %in% c('Apr', 'May', 'Jun', 'Jul', 'Aug', 'Sep', 'Oct'))] = 'Summer'
  ...
  if ('AC' %in% names(df) && 'HV' %in% names(df)) 
    dfs$AC_HV_ON = df$AC > 0 & df$HV > 0
  if ('AC' %in% names(df)) {
    dfs$AC_ON = df$AC > 0    
  }
  if ('HV' %in% names(df)) {
  ...
  # types of end uses
  end_uses = setdiff(names(data_60), c('date', 'use', 'TemperatureF'))
