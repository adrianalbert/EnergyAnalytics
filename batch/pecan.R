...
DATA_DIR = 'pecan/1-minute interval data/'
DATA_FILE = file.path(DATA_DIR,'pecan.RData')
...
pecan = function() {
  ...
  if(file.exists(DATA_FILE) && ! force) {
    ...
    pecanData = list()
    dayData = c()
    for(home in list.files(DATA_DIR)) {
      homeData = data.frame(c()) 
      for(df in list.files(file.path(DATA_DIR,home),pattern='^[^~].*xlsx')) {
        print(df)
        dayData <- read.xlsx2(file.path(DATA_DIR,home,df),1,colClasses=rep('numeric',100))
  ...
  save(list=c('pecanData'),file=DATA_FILE)
  return(pecanData)
}
...
toHourly = function(data){
  data$dayhr = strftime(data$date,'%Y-%m-%d.%H',tz='CST6CDT')
  hourly = aggregate(. ~ dayhr,data=data, mean )
...
for(homeName in names(pecanData)){
  ...
  plot(homeData$date,usage(homeData,total),type='l',col='gray',main='minute',ylim=c(0,max(usage(homeData,total))))
  ...
  plot(hrData$date,usage(hrData,total),type='l',col='gray',main='hourly',ylim=c(0,max(usage(hrData,total))))
