...
def latlong(station):
	...
	# Hack to find lat/long/elevation
	...
	startIndex = data.find("Elevation (ft):</span>") + 23
	endIndex   = data.find('</div>', startIndex, startIndex + 40) - 1
	elev			 = float(data[startIndex:endIndex])
...
for line in input:
	...
	print( "%s,%f,%f,%f"%(station,info[0],info[1],info[2]) )
