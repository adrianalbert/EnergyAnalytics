...
eurMap <- readShapePoly(fn="~/Dropbox/OccupancyStates/data/metadata/Shape (2)/data/NUTS_RG_60M_2010.shp")
eurMap@data = droplevels(subset(eurMap@data, STAT_LEVL_ == 0))

# read downloaded data (change folder appropriately)
eurEdu <- read.csv("~/Dropbox/OccupancyStates/data/metadata/educ_thexp_1_Data.csv", stringsAsFactors = F)
eurEdu$Value <- as.double(eurEdu$Value) #format as numeric
eurEdu = na.omit(eurEdu)

eurMapDf    = fortify(eurMap)
eurMapDf$id = levels(eurMap@data$NUTS_ID)[as.numeric(eurMapDf$id)+1]
