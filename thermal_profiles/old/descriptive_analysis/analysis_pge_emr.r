...
analysis_wrapper <- function(chunkID) {
  ...
  for (usr in all_users) {     
    ...
    res = try(occupancyAnalysis(cur_data, cur_wthr, usr, zip, 
                                ...
                                addl.vars = c(),
                                plots_path = plots_path_cur, dump_path = save_path_cur))  
...
# for (cur_chunk_ID in unique(usr_chunks$CHUNK)) { 
...
setCredentials('AKIAJCNEFO5KUWPMCSWA', 'sj8RLA+FjGAv4lvD50zrGjHZ8+Q6iYXCykVHTGxD')
myCluster <- createCluster(numInstances=4,
                           masterInstanceType="m1.medium",
                           slaveInstanceType="m1.medium",
                           rObjectsOnNodes = list(payments = payments, 
                                                  billing = billing))

pay.delay <- emrlapply( myCluster, chunks, computeDelayChunk )
