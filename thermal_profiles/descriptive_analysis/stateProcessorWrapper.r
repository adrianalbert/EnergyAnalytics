...
source('code/DataFormatter.r')
source('code/StateDecoder.r')
source('code/Interpreter.r')
source('code/Visualizer.r')
...
stateProcessorWrapper = function(cur_data, cur_covar, UID,  
  ...
  cur_covar = cur_covar[,-which(names(cur_covar) == 'date')]  
  ...
  interp.dump  = dumpInterpretedData(user, path = dump_path)    
