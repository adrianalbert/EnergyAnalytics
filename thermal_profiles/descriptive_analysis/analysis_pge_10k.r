...
N_USR_CHUNK = 50      # number of users in a chunk
...
load('data/selection_10k_1yr.RData')

# set up data in the format taken by the analysis wrapper
zips              = unique(consumption$ZIP5)
consumption       = cbind(CHUNK = rep(1:length(unique(consumption$sp_id)), each = 365), consumption)
consumption$CHUNK = consumption$CHUNK %/% N_USR_CHUNK + 1
consumption$UID   = consumption$uid
consumption$uid   = NULL
usr_chunks        = consumption
weather.ok        = weather
...
analysis_wrapper <- function(chunkID) {
  ...
  for (usr in all_users) {     
    ...
    if (!PLOTS) plots_path_cur = NULL else
       if(runif(1) >= 0.1) plots_path_cur = NULL    
